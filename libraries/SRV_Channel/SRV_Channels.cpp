...
const AP_Param::GroupInfo SRV_Channels::var_info[] = {
    ...
    // @Description: This enables automatic servo trim in flight. Servos will be trimed in stabilized flight modes when the aircraft is close to level. Changes to servo trim will be saved every 10 seconds and will persist between flights. By default auto-trim is limited to the central 40-60% of each output's range; this window can be widened or offset per servo with SERVOn_ATRM_MIN and SERVOn_ATRM_MAX.
